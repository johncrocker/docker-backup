...
mkdir -p "$targetdir"
case "$targetfile" in
*.tar.gz)tar -czf "$target" -C "$workingdir" .
;;
*.tar.bz2)tar -cf - -C "$workingdir" .|bzip2 -z "$BACKUP_COMPRESS_BZ2_OPT" - \
>"$target"
;;
*.tar.xz)tar -cf - -C "$workingdir" .|xz -z "$BACKUP_COMPRESS_XZ_OPT" - \
>"$target"
;;
...
>"$target"
;;
*.tar.bz2)tar -cf - -C "$volumepath" .|bzip2 -z "$BACKUP_COMPRESS_BZ2_OPT" - \
>"$target"
;;
*.tar.xz)tar -cf - -C "$volumepath" .|xz -z "$BACKUP_COMPRESS_XZ_OPT" - \
>"$target"
;;
...
-v "$volume":/source:ro \
"$BACKUP_DOCKERIMAGE" \
tar -cf - -C /source/ .|gzip "$BACKUP_COMPRESS_GZ_OPT" - \
>"$target"
;;
*.tar.bz2)docker run --rm --name volumebackup \
-v "$volume":/source:ro \
-e BACKUP_COMPRESS_BZ2_OPT="$BACKUP_COMPRESS_BZ2_OPT" \
"$BACKUP_DOCKERIMAGE" \
tar -cf - -C /source/ . bzip2 -z "$BACKUP_COMPRESS_BZ2_OPT" - \
>"$target"
;;
*.tar.xz)docker run --rm --name volumebackup \
-v "$volume":/source:ro \
-e BACKUP_COMPRESS_XZ_OPT="$BACKUP_COMPRESS_XZ_OPT" \
"$BACKUP_DOCKERIMAGE" \
tar -cf - -C /source/ .|xz -z "$BACKUP_COMPRESS_XZ_OPT" - \
>"$target"
;;
...
mkdir -p "$targetdir"
case "$targetfile" in
*.tar.gz)tar -czf "$target" -C "$source" .
;;
*.tar.bz2)tar -cf - -C "$source" .|bzip2 -z "$BACKUP_COMPRESS_BZ2_OPT" - \
>"$target"
;;
*.tar.xz)tar -cf - -C "$source" .|xz -z "$BACKUP_COMPRESS_XZ_OPT" - \
>"$target"
;;
